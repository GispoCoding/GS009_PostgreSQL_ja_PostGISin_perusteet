...
Osiossa käytetään aiemmin luotua `koulutus`-tietokantaa ja siihen ladattua `etunimet` taulua. Jos et ennättänyt niitä luoda, niin pääset samalle viivalle antamalla seuraavat sql-komennot:

::: code-box
```sql
DROP DATABASE IF EXISTS koulutus;
CREATE DATABASE koulutus TEMPLATE koulutus_template;
```
:::
Tämä luo uuden koulutus tietokannan käyttäen mallina `koulutus_template` -tietokantaa.
...
* __CREATEDB__ valitsin määrittää oikeudet roolille luoda tietokantoja.
* __VALID__ määrittää kuinka pitkään rooli on voimassa.
...
Uusi yp-roolilla on ylläpidon oikeudet ja sen salasana on voimaassa 1. tammikuun 2021 asti. Kun haluat nähdä palvelimessa olemassa roolit, käytä psql-komentoa: `\du`.
...
## 6.2 Käyttöoikeuksien lisääminen
...
Kirjaudu ensin `koulutus`-tietokantaan matin tunnuksilla.
Voit ottaa yhteyttä joko `psql`:llä tai pgAdminilla. PgAdminissa luo uusi tietokantayhteys luvun [Tietokantayhteyden lisääminen] ohjeiden mukaisesti ja anna käyttäksi matti.
Psql:llä anna matin käyttäjätunnus `-U`-parametrilla.
...
```
:::
Nyt lukemisen sekä uusien skeemojen luonnin pitäisi onnistua.

Luo nyt matti-käyttäjänä skeema matin_skeema ja sinne taulu matit. Poista samalla `public`-roolilta oikeudet tauluun. `pubic`-rooliin kuuluu siis kaikki tietokannan käyttäjät.
...
::: commandline-box
```
psql -h dbhost -d koulutus -U matti 
```
:::
