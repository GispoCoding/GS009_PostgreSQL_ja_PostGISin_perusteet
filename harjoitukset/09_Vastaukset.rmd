...
[Takaisin harjoitukseen][Harjoitus 3.2 Taulun luominen]


## Harjoitus 4.1 {#vastaus-h4.1 .unnumbered}

```sql
ALTER TABLE ajoneuvorekisteri ALTER COLUMN ensirekisterointipvm TYPE DATE 
USING to_date(ensirekisterointipvm, 'YYYY-MM-DD');
```
## Harjoitus 4.2 {#vastaus-h4.2 .unnumbered}
```sql
UPDATE ajoneuvorekisteri SET kayttoonottopvm = Null WHERE kayttoonottopvm = 0;

ALTER TABLE ajoneuvorekisteri ALTER COLUMN kayttoonottopvm TYPE DATE 
USING to_date(kayttoonottopvm::text, 'YYYYMMDD');
```
[Takaisin harjoitukseen][Harjoitus 4.1: Ota oikea tietotyyppi käyttöön]
...
[Takaisin harjoitukseen][Harjoitus 7.1: csv-tiedostosta lukeminen]
...
```
[Takaisin harjoitukseen][Harjoitus 7.2: Toisesta PostgreSQL-kannasta lukeminen]
