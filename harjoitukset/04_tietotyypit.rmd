...
Tutustutaan aluksi tieliikenne-taulun sisältöön
...
```{sql, eval=FALSE}
SELECT count(*) FROM tieliikenne WHERE EXTRACT(YEAR FROM to_date(ensirekisterointipvm, 'YYYY-MM-DD')) > 2015;
```

Tietotyyppimuunnoksen (castaus) voi tehdä myös käyttäen ::-operaattoria:

```{sql, eval=FALSE}
SELECT count(*) FROM tieliikenne WHERE extract(YEAR FROM ensirekisterointipvm::date) > 2015;
```
...
Muuta tieliikenne-taulun ensirekisterointipvm-sarake käyttämään oikeaa date-tietotyyppiä. Käytä syntaksia:
...
```{sql, eval=FALSE}
SELECT count(*) FROM tieliikenne WHERE EXTRACT(YEAR FROM ensirekisterointipvm) > 2015;
```
...
```{sql, eval=FALSE}
SELECT count(*) FROM tieliikenne WHERE EXTRACT(YEAR FROM kayttoonottopvm) > 2015;
```
...
Nyt paljastuu jo, että kyseessä on polygoni, tai tarkemmin multipolygoni, joka sisältää paljon koordinaatteja. Harjoitusympäristöön on rakennettu toiminnalisuus, jolla geometriakohteita pystyy visualisoimaan interaktiivisesti
